...
template <typename T> static bool useTexture(dim3 dg, dim3 db);

template <typename T> static bool test1_texture(int w, int h, dim3 dg, dim3 db);

template <typename T> static bool test2_texture(int w, int h, dim3 dg, dim3 db);

// ---------------------------------
//  device
// ---------------------------------

template <typename T> __global__ void fillGM(T *tabGM, int n);

template <typename T> __global__ void checkGM(cudaTextureObject_t tex2dGM, int w, int h, int *ptrSumOkGM);
...
template <typename T> bool useTexture()
    ...
    return useTexture<T>(dg, db);
    }

/**
 * exemple float int uchar char
 */
template <typename T> bool useTexture(dim3 dg, dim3 db)
    {
    ...
    for (int w = MIN; w <= MAX; w = w * 2) // *2 ok mais +2 ko // puissance de 2 depuis 8
        {
        for (int h = 1; h <= MAX; h = h + 1) // n'importe quoi
            {
            // cout << "(w,h)=(" << w << "," << h << ")"<<endl;

            bool isOkLocal = test1_texture<T>(w, h, dg, db);
            isOkLocal &= test2_texture<T>(w, h, dg, db);
...
template <typename T> bool test1_texture(int w, int h, dim3 dg, dim3 db)
    {
    const int WH = w * h;
    ...
    checkGM<T><<<dg, db, sizeSM>>>(texture2D.texCuda, w, h, ptrSumOkGM);
...
template <typename T> bool test2_texture(int w, int h, dim3 dg, dim3 db)
    {
    const int WH = w * h;
...
template <typename T> __global__ void fillGM(T *tabGM, int WH)
    {
    ...
    //    if(TID==0)
    //	{
    //	printf("kernel fillTexture WH=%d\n",WH);
    //	}
