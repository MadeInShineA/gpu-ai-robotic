 ...
 * static
 */
int MontecarloThread::NB_DEVICE = Hardware::getDeviceCount();

// ---------------------------------------------------------------------------
// Constructors
// ---------------------------------------------------------------------------

MontecarloThread::MontecarloThread(const Grid &grid, entier nbDarTotalAsk, double *ptrPiHat, float h, bool isVerbose)
...
void MontecarloThread::run()
    ...
    *this->ptrPiHat = -1;
    ...
    //			nvidia-smi --loop=1
    //
    // 		pour voir le 100% d'utilisation du device, changer de device en device dans le cas sequentiel, puis voir un 100% simultaner sur tous les
    // devices en meme temps 		Tirer assez de flechette (cas long) pour avoir le temps de monitorer (cf entier.h du to Montecarlo).
    //
...
entier MontecarloThread::getNbDarTotalEffective()
    {
    return nbDarTotalEffective;
    }

double MontecarloThread::getInputGO()
    {
...
void MontecarloThread::setH(float h)
    {
    this->h = h;
    }
...
string MontecarloThread::title(entier nbDarTotalAsk, float h)
    {
