...
class Rippling3DMath
  ...
  public:
     ...
     * </pre>
     */
    __device__ void xyz(uchar4 *ptrColorIJGM, float3 *ptrSommetXYZGM, int i, int j)
        {
        // on calcul levelGray qu une seule fois, puis on l'utilise pour :
        //	- la "couleur"
        //	- la hauteur

        float levelGrayFloat0255 = levelGray(i, j); // float car la surface sera plus "lisse" en float qu'en uchar et z peut etre en float.
            ...
            // sommet en float3 : float3 est une structure a 3 champs x y et z: 3 float
            {
            // Indication:
            ...
            //
            // Esthetique : pour la hauteur z utiliser 6 x levelGrayFloat0255s (plus joli)
  ...
  private:
    ...
    __inline__ __device__ float levelGray(int i, int j) // en float in [0,255]
        {
        // TODO RIPPLING 3D
        return -1;
        }

    __inline__ __device__ void dij(int i, int j, float *ptrResult)
        {
        // TODO RIPPLING 3D
        }
